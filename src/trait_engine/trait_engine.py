...
class TraitEngine:
    ...
    def _prepare_trait_tensors(self, trait_data: TraitData) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        trait_tensor = torch.zeros(num_traits, 2)
        trait_indices = torch.zeros(num_traits, dtype=torch.long)
        
        # Populate tensors with trait data
        for i, (trait_type, trait_vector) in enumerate(trait_data.trait_matrix.traits.items()):
            trait_tensor[i, 0] = trait_vector.value      # Trait value
            trait_tensor[i, 1] = trait_vector.confidence # Trait confidence
            trait_indices[i] = list(TraitType).index(trait_type)  # Use enum index as integer
    ...
    def _process_network_outputs(self, outputs: Dict[str, torch.Tensor], 
        ...
        interaction_weights = outputs['interaction_weights'].cpu().numpy()
